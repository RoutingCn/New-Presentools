...
class WebContractTest(unittest.TestCase):
    def test_workspace_has_stable_regions_and_agent_actions(self):
        ...
        self.assertIn('id="lock-html-preview-area"', html)

    def test_topbar_button_is_html_preview(self):
        html = Path("web/index.html").read_text(encoding="utf-8")

        self.assertIn("生成 HTML 预览", html)
    ...
    def test_client_calls_each_vertical_slice_endpoint(self):
        ...
        self.assertIn("/api/html-provider", script)
        self.assertIn("/html/preview", script)
        self.assertIn("/lock", script)
        self.assertIn("generateHtmlPreview", script)
    ...
    def test_client_has_state_management(self):
        script = Path("web/app.js").read_text(encoding="utf-8")

        self.assertIn("appState", script)
        self.assertIn("onChange", script)
    ...
    def test_provider_badge_loads_safe_health_status(self):
        html = Path("web/index.html").read_text(encoding="utf-8")
        script_js = Path("web/provider-status.js").read_text(encoding="utf-8")

        self.assertIn('id="provider-status"', html)
        self.assertIn('/api/health', script_js)
        self.assertIn('health.provider==="deepseek"', script_js)
        self.assertIn('本地模拟', script_js)

    def test_html_preview_lock_area_exists(self):
        html = Path("web/index.html").read_text(encoding="utf-8")

        self.assertIn("lock-html-preview-area", html)
        self.assertIn("锁定当前预览为正式版", html)

    def test_html_preview_button_label_is_updated(self):
        html = Path("web/index.html").read_text(encoding="utf-8")
